...
def test_collector_naming_conventions():
    ...
    try:
        ...
        for collector_class in collectors:
            ...
            assert collector_class.__name__.endswith(
                "Collector"
            ), f"{collector_class.__name__} doesn't follow naming convention"

            # Should have docstring
            assert (
                collector_class.__doc__ is not None
...
def test_evidence_structure():
    ...
    try:
        ...
        assert "evidence" in evidence["metadata"], "Metadata missing evidence field"
        assert "sha256" in evidence["metadata"], "Metadata missing sha256 field"
