...
from auditly.api import app

# Or import individual routers for custom configuration
from auditly.api.routers import collect_router, validate_router, report_router
```
...
python -m auditly.api

# Or using uvicorn directly
uvicorn auditly.api.app:app --reload --host 0.0.0.0 --port 8000
```
...
```json
{
  ...
  "environment": "production",
  "report_type": "readiness",
  "report_path": "/tmp/auditly-readiness-abc123.html",
...
services:
  auditly-api:
    ...
    environment:
      - DATABASE_URL=postgresql://user:pass@db:5432/auditly
```
...
1. **DRY**: All API operations delegate to existing modules:
   - `operations.collect_evidence()` → wraps `auditly.collectors.*`
   - `operations.validate_evidence()` → wraps `auditly.validators.validate_controls()`
   - `operations.generate_report()` → wraps `auditly.reporting.*`
