...
This package contains the auditly CLI and core services for collecting evidence, validating controls, scanning live systems, and producing audit-ready reports.
...
### Collect
```powershell
python -m auditly collect terraform --config config.yaml --env edge --plan plan.out
python -m auditly collect github --config config.yaml --env edge --repo owner/repo --branch main
python -m auditly collect azure --config config.yaml --env edge --subscription-id <sub> --resource-group <rg> --storage-account <sa> --key-vault <kv>
```

### Validate and scan
```powershell
python -m auditly policy validate --evidence ./evidence
...
### Air-gapped bundles
```powershell
python -m auditly bundle keygen --out-dir .keys --name auditly
python -m auditly bundle create --config config.yaml --env edge --out-path evidence-bundle.tar.gz
python -m auditly bundle verify --bundle-path evidence-bundle.tar.gz --public-key-path .keys\auditly.ed25519.pub
python -m auditly bundle import --config config.yaml --env govcloud-il5 --bundle-path evidence-bundle.tar.gz --public-key-path .keys\auditly.ed25519.pub
```
