...
```powershell
python -m auditly policy conftest --target path/to/iac --out-json conftest-results.json
```
...
# Evaluate offline
python -m auditly policy wasm --wasm-file policy.wasm --input input.json --out-json wasm-results.json
```
