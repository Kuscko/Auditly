...
- Scheduler CLI added: `auditly scheduler start`, `once`, and `runs`
...
```python
from auditly.scheduler.runner import start_scheduler, run_scheduled_validation
...
```python
from auditly.scheduler.core import run_validation_job, run_validation_job_sync
...
```powershell
.\.venv\Scripts\alembic.exe -x dbUrl=postgresql+asyncpg://auditly:auditly_local_pass@localhost:5433/auditly_test upgrade head
```
...
```python
from auditly.scheduler import runner
...
.\.venv\Scripts\python.exe -m auditly scheduler start --config config.yaml --env production --cron "0 2 * * *"

# Run a single validation job immediately (useful for testing)
.\.venv\Scripts\python.exe -m auditly scheduler once --config config.yaml --env production

# View recent job runs (table output)
.\.venv\Scripts\python.exe -m auditly scheduler runs --config config.yaml --env production --limit 20

# Filter by job status
.\.venv\Scripts\python.exe -m auditly scheduler runs --config config.yaml --status failed --limit 10

# Export as JSON for scripting/monitoring
.\.venv\Scripts\python.exe -m auditly scheduler runs --config config.yaml --env production --json
```
