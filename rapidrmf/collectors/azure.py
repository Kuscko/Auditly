...
def collect_azure(
    ...
    storage_json = _run_az_command(["az", "storage", "account", "show", 
    ...
    artifacts.append(ArtifactRecord(
        ...
        metadata={"kind": "azure-storage-account", "resource": storage_account, **(extra_metadata or {})},
    ))
    
    # 2. Collect Storage Account role assignments
    print(f"Collecting storage account roles...")
    ...
    storage_roles_json = _run_az_command(["az", "role", "assignment", "list", "--scope", storage_scope])
    storage_roles_data = json.loads(storage_roles_json) if storage_roles_json else []
    storage_roles_path = _create_json_file(storage_roles_data, output_dir / "storage-role-assignments.json")
    artifacts.append(ArtifactRecord(
        ...
        metadata={"kind": "azure-role-assignments", "resource": storage_account, **(extra_metadata or {})},
    ))
    
    # 3. Collect Key Vault details
    print(f"Collecting Key Vault {key_vault}...")
    kv_json = _run_az_command(["az", "keyvault", "show", 
                                "--name", key_vault, 
                                "--resource-group", resource_group])
    ...
    print(f"Collecting Key Vault roles...")
    ...
    artifacts.append(ArtifactRecord(
        ...
        metadata={"kind": "azure-role-assignments", "resource": key_vault, **(extra_metadata or {})},
    ))
    
    # 5. Collect Activity Log (24h)
    print(f"Collecting activity log...")
    activity_json = _run_az_command(["az", "monitor", "activity-log", "list",
                                      "--resource-group", resource_group,
                                      "--offset", "24h"])
    ...
    print(f"Collecting conditional access policies...")
    try:
        ca_json = _run_az_command(["az", "rest", "--method", "GET",
    ...
    print(f"Creating evidence summary...")
    evidence_summary = {
        "encryption-config": {
        ...
        "audit-trail": {
            ...
            "type": "audit-trail",
            "sources": [
                {"type": "activity-log", "file": str(activity_path.absolute())}
            ]
        },
        "mfa-config": {
            ...
            "sources": [
                {"type": "conditional-access", "file": str(ca_path.absolute())}
            ]
        }
    }
