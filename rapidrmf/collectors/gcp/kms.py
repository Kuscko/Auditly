"""GCP Cloud KMS evidence collector for RapidRMF compliance automation.
            ...
            for location in locations:
                try:
                    ...
                    for key_ring in self.kms_client.list_key_rings(request=key_ring_request):
                        ...
                        try:
                            crypto_key_request = kms_v1.ListCryptoKeysRequest(
                                parent=key_ring.name
                            )
                            
                            for crypto_key in self.kms_client.list_crypto_keys(request=crypto_key_request):
                                key_dict = {
                                    "name": crypto_key.name,
                                    "key_ring": key_ring.name,
                                    "purpose": crypto_key.purpose.name if crypto_key.purpose else None,
                                    ...
                                    "primary_version": {
                                        "name": crypto_key.primary.name if crypto_key.primary else None,
                                        "state": crypto_key.primary.state.name if crypto_key.primary and crypto_key.primary.state else None,
                                        "protection_level": crypto_key.primary.protection_level.name if crypto_key.primary and crypto_key.primary.protection_level else None,
                                        "algorithm": crypto_key.primary.algorithm.name if crypto_key.primary and crypto_key.primary.algorithm else None,
                                        "create_time": crypto_key.primary.create_time.isoformat() if crypto_key.primary and crypto_key.primary.create_time else None,
                                    } if crypto_key.primary else None,
                                ...
                                try:
                                    ...
                                    versions = []
                                    for version in self.kms_client.list_crypto_key_versions(request=versions_request):
                                        versions.append({
                                            "name": version.name,
                                            "state": version.state.name if version.state else None,
                                            "protection_level": version.protection_level.name if version.protection_level else None,
                                            "algorithm": version.algorithm.name if version.algorithm else None,
                                            "create_time": version.create_time.isoformat() if version.create_time else None,
                                        })
                                    key_dict["versions"] = versions
                                except Exception as e:
                                    logger.warning("Error collecting versions for key %s: %s", crypto_key.name, e)
                                    key_dict["versions"] = []
                                
                                crypto_keys.append(key_dict)
                                
                        except Exception as e:
                            logger.warning("Error collecting crypto keys in key ring %s: %s", key_ring.name, e)
