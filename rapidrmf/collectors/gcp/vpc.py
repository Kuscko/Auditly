"""GCP VPC evidence collector for RapidRMF compliance automation.
...
class VPCCollector:
            ...
            for region, response in subnetworks_client.aggregated_list(request=aggregated_request):
                ...
                for subnetwork in response.subnetworks:
                    subnetwork_dict = {
                        ...
                        "enable_flow_logs": subnetwork.enable_flow_logs if hasattr(subnetwork, 'enable_flow_logs') else False,
                        "log_config": {
                            "enable": subnetwork.log_config.enable if subnetwork.log_config else False,
                            "flow_sampling": subnetwork.log_config.flow_sampling if subnetwork.log_config else None,
                            "aggregation_interval": subnetwork.log_config.aggregation_interval if subnetwork.log_config else None,
                        } if subnetwork.log_config else {},
    ...
    def collect_vpn_tunnels(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            aggregated_request = compute_v1.AggregatedListVpnTunnelsRequest(
                project=self.project_id
            )
            ...
            for region, response in vpn_tunnels_client.aggregated_list(request=aggregated_request):
                ...
                for tunnel in response.vpn_tunnels:
                    tunnel_dict = {
                        ...
                        "target_vpn_gateway": tunnel.target_vpn_gateway.split("/")[-1] if tunnel.target_vpn_gateway else None,
                        "vpn_gateway": tunnel.vpn_gateway.split("/")[-1] if tunnel.vpn_gateway else None,
    ...
    def collect_routers(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            aggregated_request = compute_v1.AggregatedListRoutersRequest(
                project=self.project_id
            )
