...
```powershell
# Terraform
python -m rapidrmf collect terraform --config config.yaml --env edge --plan plan.out --apply apply.log

# GitHub
python -m rapidrmf collect github --config config.yaml --env edge --repo owner/repo --branch main --token $env:GITHUB_TOKEN

# GitLab
python -m rapidrmf collect gitlab --config config.yaml --env edge --project-id group/proj --ref main --token $env:GITLAB_TOKEN

# Argo
python -m rapidrmf collect argo --config config.yaml --env edge --base-url https://argo-server:2746 --workflow-name my-workflow

# Azure
python -m rapidrmf collect azure --config config.yaml --env edge --subscription-id <sub> --resource-group <rg> --storage-account <sa> --key-vault <kv> --output-dir ./azure-evidence
```
