...
class CollectRequest(BaseModel):
    ...
    gitlab_project_id: Optional[str] = Field(None, description="GitLab project ID or 'namespace/project'")
...
class ValidateRequest(BaseModel):
    ...
    control_ids: Optional[List[str]] = Field(None, description="Specific control IDs to validate (default: all from catalogs)")
    evidence_dict: Optional[Dict[str, Any]] = Field(None, description="Override evidence dict (default: fetch from DB)")
...
class ReportRequest(BaseModel):
    ...
    config_path: str = Field(default="config.yaml", description="Path to config.yaml file")
    environment: str = Field(..., description="Environment key")
    report_type: str = Field(default="readiness", description="Report type: readiness, engineer, auditor")
    control_ids: Optional[List[str]] = Field(None, description="Specific control IDs for engineer/auditor reports")
    evidence_dict: Optional[Dict[str, Any]] = Field(None, description="Override evidence dict for engineer/auditor reports")
