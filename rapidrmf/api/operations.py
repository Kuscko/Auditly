...
from ..validators import validate_controls, get_control_requirement
    ...
    **provider_params
    ...
    if provider == "terraform":
        ...
        if not all([base_url, namespace, workflow_name]):
            raise ValueError("argo_base_url, argo_namespace, and argo_workflow_name are required for argo provider")
...
def validate_evidence(
    ...
    if evidence_dict is None:
        # Try to load from DB or manifests
        evidence_dict = {}
        
        # Check for staging manifests (CLI-style)
...
def _generate_readiness_report(cfg: AppConfig, environment: str, output_path: Optional[str]) -> tuple[str, str, Dict[str, Any]]:
    """Generate readiness report (CLI logic)."""
    staging = Path(".rapidrmf_manifests")
    staging.mkdir(exist_ok=True)
    
    if not any(staging.glob(f"{environment}-*.json")):
        dummy = EvidenceManifest.create(environment, [ArtifactRecord(key="noop", filename="noop", sha256="0", size=0, metadata={})])
    ...
    if output_path:
        ...
        out = Path(tempfile.mktemp(suffix=".html", prefix="rapidrmf-readiness-"))
...
def _generate_engineer_report(cfg: AppConfig, environment: str, control_ids: Optional[List[str]], evidence_dict: Optional[Dict[str, Any]], output_path: Optional[str]) -> tuple[str, str, Dict[str, Any]]:
    ...
    if output_path:
        ...
        out = Path(tempfile.mktemp(suffix=".html", prefix="rapidrmf-engineer-"))
...
def _generate_auditor_report(cfg: AppConfig, environment: str, control_ids: Optional[List[str]], evidence_dict: Optional[Dict[str, Any]], output_path: Optional[str]) -> tuple[str, str, Dict[str, Any]]:
    ...
    if output_path:
        ...
        out = Path(tempfile.mktemp(suffix=".html", prefix="rapidrmf-auditor-"))
